#ifndef MUJOCO_WITH_ROS2_RENDER_H_
#define MUJOCO_WITH_ROS2_RENDER_H_
...
static_assert( (Clock::period::num * 1'000LL) <= Clock::period::den,
  ...
  MujocoWithROS2Render(std::unique_ptr<MujocoWithRos2UIAdapter> mujoco_with_ros2_adapter,mjvCamera* cam,mjvOption* opt,mjvPerturb* pert);
...
// loop to render the UI (must be called from main thread because of MacOS)
  ...
  std::vector<std::pair<mjtNum, mjtNum>> jnt_range_;
  std::vector<std::string> jnt_names_;

  std::vector<int> actuator_group_;
  std::vector<std::pair<mjtNum, mjtNum>> actuator_ctrlrange_;
  ...
  int window_pos[2] = {0};
  int window_size[2] = {0};
  std::unique_ptr<MujocoWithRos2UIAdapter> platform_ui;
  mjuiState& uistate;
  mjUI ui0 = {};
  mjUI ui1 = {};
  ...
  // info strings
  char info_title[MujocoWithROS2Render::kMaxFilenameLength] = {0};
  char info_content[MujocoWithROS2Render::kMaxFilenameLength] = {0};
